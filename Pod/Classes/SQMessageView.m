...
#define SQMessageViewMinimumPadding 15.0

#define TSDesignFileName @"SQMessagesDefaultDesign"

static NSMutableDictionary *_notificationDesign;

@interface SQMessage (SQMessageView)
- (void)fadeOutNotification:(SQMessageView *)currentView; // private method of SQMessage, but called by SQMessageView in -[fadeMeOut]
@end

@interface SQMessageView () <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) SQBlurView *backgroundBlurView; // Only used in iOS 7
...
@implementation SQMessageView{
    SQMessageNotificationType notificationType;
}
...
-(void) updateCurrentIcon{
    ...
    {
        case SQMessageNotificationTypeMessage:
        {
            image = _messageIcon;
            self.iconImageView.image = _messageIcon;
            break;
        }
        case SQMessageNotificationTypeError:
        {
            image = _errorIcon;
            self.iconImageView.image = _errorIcon;
            break;
        }
        case SQMessageNotificationTypeSuccess:
        {
            image = _successIcon;
            self.iconImageView.image = _successIcon;
            break;
        }
        case SQMessageNotificationTypeWarning:
        {
            image = _warningIcon;
...
+ (NSMutableDictionary *)notificationDesign
{
    if (!_notificationDesign)
    {
        ...
        NSAssert(data != nil, @"Could not read SQMessages config file from main bundle with name %@.json", TSDesignFileName);
...
+ (void)addNotificationDesignFromFile:(NSString *)filename
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:path])
    {
        ...
        [[SQMessageView notificationDesign] addEntriesFromDictionary:design];
    }
    else
    {
...
- (CGFloat)padding
{
    ...
    return self.messagePosition == SQMessageNotificationPositionNavBarOverlay ? SQMessageViewMinimumPadding + 10.0f : SQMessageViewMinimumPadding;
}

- (id)initWithTitle:(NSString *)title
           subtitle:(NSString *)subtitle
              image:(UIImage *)image
               type:(SQMessageNotificationType)aNotificationType
           duration:(CGFloat)duration
   inViewController:(UIViewController *)viewController
           callback:(void (^)())callback
        buttonTitle:(NSString *)buttonTitle
     buttonCallback:(void (^)())buttonCallback
         atPosition:(SQMessageNotificationPosition)position
canBeDismissedByUser:(BOOL)dismissingEnabled
{
    NSDictionary *notificationDesign = [SQMessageView notificationDesign];
    ...
    if ((self = [self init]))
    {
        ...
        else
        {
            ...
            _backgroundBlurView = [[SQBlurView alloc] init];
        ...
        if (self.messagePosition == SQMessageNotificationPositionBottom)
        {
        ...
        if (self.messagePosition == SQMessageNotificationPositionTop)
        {
        ...
        if (dismissingEnabled)
        {
            ...
            [gestureRec setDirection:(self.messagePosition == SQMessageNotificationPositionTop ?
...
- (CGFloat)updateHeightOfMessageView
{
    ...
    if ([SQMessage iOS7StyleEnabled])
    {
        if (self.messagePosition == SQMessageNotificationPositionTop)
        {
            ...
            BOOL isNavBarIsHidden = !navigationController || [SQMessage isNavigationBarInNavigationControllerHidden:navigationController];
        ...
        else if (self.messagePosition == SQMessageNotificationPositionBottom)
        {
...
- (void)fadeMeOut
{
    [[SQMessage sharedMessage] performSelectorOnMainThread:@selector(fadeOutNotification:) withObject:self waitUntilDone:NO];
}

- (void)didMoveToWindow
{
    [super didMoveToWindow];
    if (self.duration == SQMessageNotificationDurationEndless && self.superview && !self.window )
    {
